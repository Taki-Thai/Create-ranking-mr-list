...
- **Already-sent filter (this month):** before writing, `office_ids_sent_this_month()` reads the **`Log`** sheet (same spreadsheet; columns `送信日時, officeUserId, MR名, Pattern, suggest病院, HTTPステータス, 結果`). Any id whose `送信日時` falls in the **current month** is dropped from `s_rank_df`, so a user contacted earlier this month is not sent again. The Log's `officeUserId` is matched to `s_rank_df.officeuserid` case-insensitively. Fails safe (skips the filter) if the `Log` sheet or its `officeUserId`/`送信日時` columns are missing. The filtered `s_rank_df` is what flows into the Pattern/Case steps too.
