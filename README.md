...
- **Already-sent filter (last 30 days):** before writing, `office_ids_sent_recently()` reads the **`Log`** sheet (same spreadsheet; columns `送信日時, officeUserId, MR名, Pattern, suggest病院, HTTPステータス, 結果`). Any id whose `送信日時` is within the **last `SENT_WINDOW_DAYS` (30) days** up to now is dropped from `s_rank_df`, so a user contacted in the past 30 days is not sent again. The Log's `officeUserId` is matched to `s_rank_df.officeuserid` case-insensitively. Fails safe (skips the filter) if the `Log` sheet or its `officeUserId`/`送信日時` columns are missing. The filtered `s_rank_df` is what flows into the Pattern/Case steps too.
