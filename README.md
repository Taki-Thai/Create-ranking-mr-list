...
- **Already-sent filter (this month):** before writing, `office_ids_sent_this_month()` reads the **`Log`** sheet (same spreadsheet). Any `officeuserid` whose `送信日時` falls in the **current month** is dropped from `s_rank_df`, so a user contacted earlier this month is not sent again. Fails safe (skips the filter) if the `Log` sheet or its `officeuserid`/`送信日時` columns are missing. The filtered `s_rank_df` is what flows into the Pattern/Case steps too.
